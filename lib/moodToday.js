...
export function setMoodToday(value, note){
  ...
  // Normalize score to 1..5 as used across the app (Eu360 maps back to 0..4)
  const v = Number(value);
  const score = Math.max(1, Math.min(5, Math.round(v + 1)));
...
export function getMoodToday(){
  ...
  for (let i = list.length - 1; i >= 0; i--) {
    ...
    if (Number.isFinite(ts) && dayStamp(new Date(ts)) === today) {
      ...
      const value = Number.isFinite(score) ? Math.max(0, Math.min(4, Math.round(score - 1))) : 0;
