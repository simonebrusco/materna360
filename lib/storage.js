...
export function getLastContext(defaultVal = "Casa") {
  try { if (!IS_BROWSER) return defaultVal; const { safeGet } = require('@/lib/utils/safeStorage'); const v = safeGet("m360.lastCtx", defaultVal); return v || defaultVal; } catch { return defaultVal; }
}
export function setLastContext(v) { try { if (!IS_BROWSER) return; const { safeSet } = require('@/lib/utils/safeStorage'); safeSet("m360.lastCtx", v); } catch {} }
export function getLastRecCategory(defaultVal = "Livros") {
  ...
  try { if (!IS_BROWSER) return defaultVal; const { safeGet } = require('@/lib/utils/safeStorage'); const x = safeGet("m360.lastRecCat", defaultVal); return x || defaultVal; } catch { return defaultVal; }
}
export function setLastRecCategory(v) {
  if (typeof v !== 'string' || !v) return;
  set(keys.recProductsCategory, v);
  try { if (IS_BROWSER) { const { safeSet } = require('@/lib/utils/safeStorage'); safeSet("m360.lastRecCat", v); } } catch {}
}
...
export function getWeeklyPlan(){
  ...
  if (IS_BROWSER) {
    try {
      ...
      const arr = safeGet('weeklyPlan', null);
