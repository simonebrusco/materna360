...
// Simple last selections (explicit keys)
export function getLastAge(){
  if (typeof window==='undefined') return '3-4';
  try { return window.localStorage.getItem('lastAge') || '3-4'; } catch { return '3-4'; }
}
export function setLastAge(age){
  try { if (typeof window!=='undefined') window.localStorage.setItem('lastAge', age); } catch {}
}
export function getLastPlace(){
  if (typeof window==='undefined') return 'Casa';
  try { return window.localStorage.getItem('lastPlace') || 'Casa'; } catch { return 'Casa'; }
}
export function setLastPlace(place){
  try { if (typeof window!=='undefined') window.localStorage.setItem('lastPlace', place); } catch {}
}
