...
export function isBrowser(){ return typeof window !== "undefined" && typeof localStorage !== 'undefined'; }
function getWin(){ try { return isBrowser() ? window : null; } catch { return null; } }
...
export const keys = {
  ...
  // premium / downloads (PR12)
  premium: `${NS}premium`,
  downloads: `${NS}downloads`,
  premiumContent: `${NS}premiumContent`,

  // reminders (PR14)
  reminders: `${NS}reminders`, // { breathe:{on:true,everyMin:180}, gratitude:{on:true,at:['20:00']} }
  lastSeen:  `${NS}lastSeen`,  // { breathe: ts?, gratitude: 'YYYY-MM-DD:HH:MM'? }
};
...
export function set(key, value) {
  ...
  if (win) {
    queueMicrotask(() => {
      win.dispatchEvent(new CustomEvent(UPDATED_EVENT, { detail: { key } }));
    });
  }
}
...
export function onUpdate(handler) {
  ...
  win.addEventListener(UPDATED_EVENT, h);
  return () => win.removeEventListener(UPDATED_EVENT, h);
}
...
export function migrateV1() {
  ...
  try {
    ...
    if (moved) {
      queueMicrotask(() => win.dispatchEvent(new CustomEvent(UPDATED_EVENT, { detail: { key: 'migration' } })));
...
export function getAll() {
  return {
    ...
    // PR12
    premium: get(keys.premium, { active: false }),
    downloads: get(keys.downloads, { items: [], countN: {} }),
    premiumContent: get(keys.premiumContent, []),

    // PR14
    reminders: get(keys.reminders, { breathe:{ on:true, everyMin:180 }, gratitude:{ on:true, at:['20:00'] } }),
    lastSeen: get(keys.lastSeen, {}),
  };
}

// Aliases and default export for compatibility with older imports
export function getItem(key, fallback){ return get(key, fallback); }
export function setItem(key, value){ return set(key, value); }
export function canUseStorage(){ return isBrowser(); }

const storage = { keys, get, set, onUpdate, getAll, migrateV1, readJSON, writeJSON, getItem, setItem, canUseStorage };
