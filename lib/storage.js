...
export function getLastRecCategory(defaultVal = "Livros") {
  try { const v = IS_BROWSER ? window.localStorage.getItem("m360.lastRecCat") : null; return v || defaultVal; } catch { return defaultVal; }
}
export function setLastRecCategory(v) { try { if (IS_BROWSER) window.localStorage.setItem("m360.lastRecCat", v); } catch {} }
...
export const getLastProductsCategory = () => getJsonKV("m360:lastProductsCategory", "livros");
export const setLastProductsCategory = (v) => setJsonKV("m360:lastProductsCategory", v);
