// Ideas generator (JS-only)

/** @typedef {{ id: string, title: string, desc: string, duration?: string }} Idea */

/** @type {Record<string, Record<string, Idea[]>>} */
const bank = {
  "0-2": {
    Casa: [
...
export function generateIdeas(age, context){
  const ctx = (bank?.[age] && bank?.[age]?.[context]) ? bank[age][context] : [];
