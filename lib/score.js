import { getAll, get as storageGet, keys } from './storage';
...
export function compute({ actions = [], gratitudes = [], planner = {} } = {}) {
  ...
  // Mood average over last 7 days (0..5)
  let moodPts = 0;
  try {
    const moods = Array.isArray(storageGet(keys.moodHistory, [])) ? storageGet(keys.moodHistory, []) : [];
    const today = new Date();
    const start = new Date(today.getFullYear(), today.getMonth(), today.getDate());
    start.setDate(start.getDate() - 6);
    const startKey = start.toISOString().slice(0, 10);
    const toKey = (m) => {
      if (typeof m?.date === 'string' && /^\d{4}-\d{2}-\d{2}/.test(m.date)) return m.date.slice(0,10);
      const t = Number(m?.ts) || Date.parse(m?.date);
      return Number.isFinite(t) ? new Date(t).toISOString().slice(0,10) : '';
    };
    const valOf = (m) => {
      const v = Number(m?.value ?? m?.score ?? 0);
      if (v >= 1 && v <= 5) return v;
      if (v >= 0 && v <= 4) return v;
      return 0;
    };
    const last7 = moods.filter((m) => toKey(m) >= startKey);
    if (last7.length) {
      const sum = last7.reduce((a, b) => a + valOf(b), 0);
      const moodAvg = sum / last7.length;
      moodPts = Math.round(moodAvg * 6); // 0..30
    }
  } catch {}

  // Routine points from daily plan over last 7 days (0..63)
  let routinePts = 0;
  try {
    const all = storageGet(keys.dailyPlan, {}) || {};
    ...
    if (days.length) {
      ...
      routinePts = routine * 3; // 0..63
    }
  } catch {}

  const raw = actionsPts + gratPts + planPts + moodPts + routinePts;
